...
document.addEventListener('DOMContentLoaded', async () => {
    try {
            ...
            data.experience.items.forEach(item => {
                ...
                <div class="timeline-item reveal active">
                    ...
                    <div class="timeline-content glass-card">
                        <span class="date">${escapeHtml(item.date[lang])}</span>
                        <h3>${escapeHtml(item.title[lang])}</h3>
                        <h4>${escapeHtml(item.company[lang])}</h4>
        ...
        if (booksListContainer && data.books.items) {
            ...
            data.books.items.forEach(book => {
                ...
                <div class="book-item glass-card card-hover">
                    <i class="fas ${escapeHtml(book.icon)} book-icon"></i>
                    <div class="book-info">
                        <h3>${escapeHtml(book.title[lang])}</h3>
                        <p>${escapeHtml(book.description[lang])}</p>
                    </div>
                </div>`;
            });
        }
        ...
        // Portfolio Grid Items (rendered from CMS when items exist)
        const portfolioGrid = document.querySelector('.portfolio-grid');
        if (portfolioGrid && data.portfolio.items && data.portfolio.items.length > 0) {
            portfolioGrid.innerHTML = '';
            data.portfolio.items.forEach(item => {
                const category = escapeHtml(item.category || 'social');
                const imgSrc = escapeHtml(item.image);
                const titleText = escapeHtml(item.title[lang]);
                const subtitleText = escapeHtml(item.subtitle[lang]);
                portfolioGrid.innerHTML += `
